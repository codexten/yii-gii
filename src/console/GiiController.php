...
class GiiController extends \yii\console\Controller
{
    ...
    public function actionMigration($repo)
    {
    ...
    public function actionCrud($repo)
    {
        ...
        foreach ($config['crud'] as $tableName => $crud) {
            $modelClass = Inflector::camelize($tableName);
            $params = ArrayHelper::merge($defaultParams, [
                'apiControllerClass' => "{$config['namespace']}api\\{$modelClass}Controller",
                'controllerClass' => "{$config['namespace']}controllers\\{$modelClass}Controller",
                'modelClass' => "{$config['namespace']}models\\{$modelClass}",
                'viewPath' => $this->getNamespacePath($config['namespace']) . 'views/' . Inflector::camel2id($modelClass),
            ]);
            $params = ArrayHelper::merge($params, $crud);
            $params = ArrayHelper::merge($this->defaultParams, $params);
    ...
    {
        exec("chown -R www-data:www-data /app /en");
